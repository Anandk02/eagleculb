...
const recharge = async (req, res) => {
    ...
    if (type != 'cancel') {
        if (!auth || !money || money <= 50) {
            return res.status(200).json({
                message: 'Add min Balance ₹ 50 ',
                status: false,
                timeStamp: timeNow,
            })
        }
...
const addBanktrx = async (req, res) => {
    let auth = req.cookies.auth;
    let name_bank = req.body.user_trx;
    ...
    const [user] = await connection.query('SELECT `phone`, `code`,`name_user` FROM users WHERE `token` = ? ', [auth]);
    let userInfo = user[0];
    console.log(">>>>>>>>>>>>>>>>"+user);
    console.log(">>>>>>>>>>>>>>>>"+userInfo.phone);
    console.log(">>>>>>>>>>>>>>>>"+name_bank);
    if (!user) {
        return res.status(200).json({
            message: 'Failed',
            status: false,
    ...
    const [user_bank2] = await connection.query('SELECT * FROM user_trx WHERE phone = ? ', [userInfo.phone]);

    if (user_bank2.length == 0) {
        let time = new Date().getTime();
        const sql = `INSERT INTO user_trx SET 
        phone = ?,
        name_user = ?,
        email = ?,
        trx_address = ?,
        time = ?`;
        await connection.execute(sql, [userInfo.phone, userInfo.name_user, userInfo.code, name_bank, time]);
        return res.status(200).json({
            message: 'Added TRX Address successfully',
            status: true,
            timeStamp: timeNow,
        });
    } 
        ...
        return res.status(200).json({
            message: 'The trx address has already exist',
            status: false,
            timeStamp: timeNow,
        });
    }
...
const addBank = async (req, res) => {
    ...
    if (user_bank.length == 0 && user_bank2.length == 0) {
        ...
        chi_nhanh = ?,
        time = ?`;
        await connection.execute(sql, [userInfo.phone, name_bank, name_user, stk, tp, email, sdt, tinh,account_no, chi_nhanh, time]);
        return res.status(200).json({
...
const infotrx = async (req, res) => {
    ...
    const [usertrx] = await connection.query('SELECT * FROM user_trx WHERE phone = ? ', [userInfo.phone]);
    return res.status(200).json({
        message: 'Get success',
        datas: usertrx,
        userInfo: user,
        ...
        if (withdraw.length < 3) {
            if (userInfo.money - money >= 0) {
                if (result == 0) {
                    ...
                    today = ?,
                    time = ?`;
                    await connection.execute(sql, [id_time + '' + id_order, userInfo.phone, money, infoBank.stk, infoBank.name_bank, infoBank.name_user, infoBank.account_no, infoBank.tp, infoBank.chi_nhanh, 0, checkTime, dates]);
...
const withdrawal4 = async (req, res) => {
    ...
    const [user_bank] = await connection.query('SELECT * FROM user_bank WHERE `phone` = ?', [userInfo.phone]);
    const [user_trx] = await connection.query('SELECT * FROM user_trx WHERE `phone` = ?', [userInfo.phone]);
    const [withdraw] = await connection.query('SELECT * FROM withdraw WHERE `phone` = ? AND today = ?', [userInfo.phone, checkTime]);
    if (user_bank.length != 0) {
        if (withdraw.length < 3) {
            if (userInfo.money - money >= 0) {
                if (result == 0) {
                    ...
                    today = ?,
                    time = ?`;
                    await connection.execute(sql, [id_time + '' + id_order, userInfo.phone, money, 'TRX DATA', 'TRX DATA', 'TRX DATA', trx.trx_address, 'TRX DATA', 'TRX DATA', 0, checkTime, dates]);
                    await connection.query('UPDATE users SET money = money - ? WHERE phone = ? ', [money, userInfo.phone]);
                    return res.status(200).json({
                        message: 'Withdraw money successfully',
                        status: true,
        ...
        return res.status(200).json({
            message: 'Please add TRX Address first',
            status: false,
            timeStamp: timeNow,
        });
    }
